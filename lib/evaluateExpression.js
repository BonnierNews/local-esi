...
module.exports = function evaluateExpression(test, context) {
  const funcs = {
    ...
    add_header([name, value]) {
      context.emit("set-header", getFunc(name.type)(name), getFunc(value.type)(value));
    },
    ...
    set_response_code([code, body]) {
      if (body) {
        return context.emit("send-body", getFunc(code.type)(code), getFunc(body.type)(body));
      }

      context.emit("status", getFunc(code.type)(code), getFunc(code.type)(code));
    },
