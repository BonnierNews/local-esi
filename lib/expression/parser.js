...
function Parser(sourceText, onStart, onEnd, columnOffset, line) {
  const lexer = new Lexer(sourceText, columnOffset, line);
  let token;
  return {
    consume,
  };
  ...
  function consume() {
    ...
    switch (node.type) {
      ...
      case ARRAY: {
        ...
        while (elm && elm.type !== "]") {
          if (elm.type === ",") return lexer.abort("Unexpected", elm.type, "in", node.type);
          ...
          if (elm.type === ",") {
            elm = consume();
          } else if (elm.type !== "]") return lexer.abort("Unexpected", elm.type, "in", node.type);
        }

        if (!elm || elm.type !== "]") return lexer.abort("Unclosed", node.type);
      ...
      case BLOCK: {
        ...
        if (!arg || arg.type !== ")") return lexer.abort("Unclosed", node.type);
      ...
      case FUNCTION: {
        ...
        let arg = consume();
        if (arg && arg.type === ",") return lexer.abort("Unexpected", arg.type, "in", node.type);
      ...
      case MEMBER: {
        ...
        const end = consume();
        if (!end || end.type !== ")") return lexer.abort("Unclosed", node.type);
      ...
      case OBJECT: {
        ...
        if (!prop || prop.type !== "}") return lexer.abort("Unclosed", node.type);
    ...
    function addObjectProperty(objectNode, commaNode, keyNode) {
      ...
      if (!keyNode) return lexer.abort("Unclosed", OBJECT);
      if (keyNode.type !== LITERAL) return lexer.abort("Unexpected key", keyNode.type);

      const colon = consume();
      if (!colon) return lexer.abort("Missing key value separator");
      if (colon.type !== ":") return lexer.abort("Unexpected", colon.type, "in object");
