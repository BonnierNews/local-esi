...
module.exports = function evaluate(ast, context) {
  const funcs = {
    ...
    add_header([name, value]) {
      context.emitter.emit("add_header", getFunc(name.type)(name), getFunc(value.type)(value));
    },
    set_redirect([location]) {
      context.emitter.emit("set_redirect", 302, getFunc(location.type)(location));
      context.redirected = true;
    },
    set_response_code([code, body]) {
      if (body) {
        return context.emitter.emit("set_response_code", getFunc(code.type)(code), getFunc(body.type)(body));
      }

      context.emitter.emit("set_response_code", getFunc(code.type)(code));
    },
    str([arg]) {
