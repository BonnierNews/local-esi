...
module.exports = function evaluate(ast, context) {
  const funcs = {
    ...
    set_response_code([code, markup]) {
      const statusCode = getFunc(code.type)(code);
      const payload = markup && getFunc(markup.type)(markup);

      if (!context.callbackMode) return context.res.status(statusCode).send(payload || "");
