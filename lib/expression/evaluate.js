...
class Evaluator {
  ...
  Identifier(node, nodeContext) {
    if (node.name === "var2") console.log(node, this.context.tries, this.context.assigns);
    if (!nodeContext) {
      if (this.context.tries.length) {
        const locals = this.context.tries.reduce((acc, curr) => {
          return {
            ...acc,
            ...curr.locals,
          };
        ...
        nodeContext = {
          ...this.context.assigns,
          ...locals,
        };
      } else {
        nodeContext = this.context.assigns;
      }
    }
