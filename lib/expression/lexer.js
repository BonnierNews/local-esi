...
class Token {
  constructor(startChar, columnOffset = 0, line = 1) {
...
class Lexer {
  ...
  consumeFunction(token) {
    ...
    if (token.type === IDENTIFIER && c === "{") {
      ...
      if (c !== "(") abort({column: this.column}, "Unexpected", c ? "char " + c : "end of line");
  ...
  consumeSingleQuoute(token) {
    ...
    if (c3 === "'''") {
      ...
      while (c !== quoteChars) {
        if (c === undefined) token.abort("Found end of file before end of string literal");
        else if (c === "\\") {
          c = this.getChar().c;
        }
  ...
  consumeTwoCharacters(token) {
    ...
    if (this.c3 === undefined || this.column === 0) token.abort("Unexpected token", c2);
  ...
  consumeOneCharacter(token) {
    ...
    if (c1 === "|" && (this.c2 === undefined || this.column === 0)) token.abort("Unexpected token |");
  ...
  get() {
    const token = this.token = new Token(this.char, this.columnOffset, this.line);
    const c1 = this.c1;
    if (c1 === undefined) {
      token.type = ENDMARK;
      return this.next();
    ...
    return abort({column: this.column}, "Unexpected token", c1);
  }
  next() {
    let c = this.c1;
    while (c === " ") {
