...
function Lexer(str, sourceMap) {
  ...
  function get() {
    ...
    if (WHITESPACE_CHARS.indexOf(c1) > -1) {
    ...
    if (c1 === IDENTIFIER_STARTCHAR) {
      ...
      if (sourceMap) {
        switch (token.type) {
          case IDENTIFIER:
            return next(IDENTIFIER_STARTCHAR, "(", token.cargo, ")");
          case OBJECT:
            return next(IDENTIFIER_STARTCHAR, "(", token.cargo, "{");
          case FUNCTION:
            return next(IDENTIFIER_STARTCHAR, token.cargo, "(");
    ...
    function next(...sourceArgs) {
      if (sourceMap) {
        let source = sourceArgs.length ? sourceArgs.join("") : token.cargo;
        while (WHITESPACE_CHARS.indexOf(c1) > -1) {
          source += c1;
          getChar();
        }
