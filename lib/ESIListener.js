...
module.exports = function ESIListener(context) {
  ...
  function handleProcessingInstructions(text) {
    ...
    console.log({text})
    const expressions = evaluateExpression(text, context, true);
    ...
    //       inExpression = false;
    //       let expressionResult = evaluateExpression(text.substring(expressionStart, i + 1), context);
    //       if (expressionResult !== undefined) {
    //         if (Array.isArray(expressionResult)) {
    //           expressionResult = `[${expressionResult.join(", ")}]`;
    //         }
