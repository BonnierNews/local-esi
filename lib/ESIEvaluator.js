...
module.exports = function ESIEvaluator(context) {
  ...
  esiTags["esi:foreach"] = {
    ...
    close(next) {
      context.inEsiStatementProcessingContext = false;
      const itemChunks = context.items.reduce((allItemChunks, item) => {
        context.assigns.item = item;
        return allItemChunks.concat(context.itemChunks.map((chunk) => {
          return (typeof chunk === "function") ? chunk(context) : chunk;
        }));
      }, []);

      delete context.items;
      delete context.itemChunks;
      delete context.assigns.item;
      writeToResult(itemChunks, next);
    }
  };
