...
class ESITry extends ESITag {
  ...
  open(data, next) {
    this.context.tries.push({ success: true, chunks: [], locals: {} });
    return next();
  }
  close(next) {
...
class ESIAttempt extends ESITag {
  ...
  close(next) {
    ...
    this.context.tries.at(-1).locals = {};
...
class ESIAssign extends ESITag {
  open(data, next) {
    ...
    try {
      value = assign(value, context);
    } catch (err) {
      if (!/unknown keyword/i.test(err.message)) return next(err);
    }

    const vars = context.tries.at(-1)?.locals || context.assigns;
