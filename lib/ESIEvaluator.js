...
const {transform: transformHtml, convert, createESIParser} = require("./transformHtml");
  ...
  esiTags["esi:include"] = {
    open(data, next) {
      ...
      fetchIncluded(data, (fetchError, fetchResult) => {
        ...
        const chunks = [];
        let stream = convert(fetchResult).pipe(new HtmlParser({ preserveWS: true }));
        if (data.dca === "esi") {
          stream = stream.pipe(createESIParser(listener))
        }
