...
class ESITry extends ESITag {
  ...
  close(next) {
    ...
    for (const chunk of chunks) {
      this.context.writeToResult(chunk, next);
    }
...
export default class ESIEvaluator {
  ...
  onopentag(name, data, next) {
    ...
    if (context.tries.length) {
      context.tries.at(-1).chunks.push({ name, data });
      return next();
    }
  ...
  onclosetag(name, next) {
    ...
    if (name !== "esi:try" && context.tries.length) {
      context.tries.at(-1).chunks.push({ name });
      return next();
    }
  ...
  ontext(text, next) {
    ...
    if (context.tries.length) {
      context.tries.at(-1).chunks.push({ text });
      return next();
    }
