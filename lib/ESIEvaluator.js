...
const {transform: transformHtml} = require("./transformHtml");
...
module.exports = function ESIListener(context) {
  ...
  esiTags["esi:include"] = {
    open(attribs, next) {
      ...
      fetchIncluded(attribs, (fetchError, fetchResult) => {
        ...
        const listener = ESIListener(ListenerContext(context.req, context.res, context.emitter));
  ...
  function onopentag(tagname, attribs, next) {
    ...
    if (tagname.startsWith("esi:")) {
      ...
      if (selfClosingElements.includes(tagname)) {
        return writeToResult({tagname, attribs: makeAttributes(attribs)}, next);
      }
    }
    ...
    writeToResult({tagname, attribs: makeAttributes(attribs)}, next);
  }
  ...
  function handleProcessingInstructions(text) {
    text = text.replace(/\$set_response_code\(\s*(\d{3})\s*,\s*(')((.*?)\2\s*\))?/ig, (_, responseCode, _2, _3, content) => { // set_response_code with replacement markup
      ...
      context.emit("status", parseInt(responseCode));
  ...
  function onclosetag(tagname, next) {
    ...
    if (tagname === "!--") {
      return writeToResult({tagname}, next);
    }

    writeToResult({tagname}, next);
  }

  function makeAttributes(attr) {
    if (!attr) return {};
  ...
  function writeToResult(chunk, next) {
    ...
    console.log("writeToResult", chunk)
