...
module.exports = function esiExpressionParser(str) {
  ...
  const binaryComparisonMatch = str.match(/(.+?)(==|!=|>=|<=)(.+)/);
  ...
  if (callExpressionMatch) {
    return {
      ...
      arguments: [esiExpressionParser(callExpressionMatch[2])]
    };
  }

  const arithmeticExpressionMatch = str.match(/(.+?)(\*|\%|\+)(.+)/);
  if (arithmeticExpressionMatch) {
    return {
      type: "ArithmeticExpression",
      operator: arithmeticExpressionMatch[2],
      left: esiExpressionParser(arithmeticExpressionMatch[1]),
      right: esiExpressionParser(arithmeticExpressionMatch[3])
    };
  }
