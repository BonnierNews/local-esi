...
export default class ListenerContext {
  ...
  async fetch(data) {
    ...
    if (!fetchUrl.startsWith("http")) {
      ...
      // if this is localhost and we have a computeAtEdge config when we need to map to a backend
      if (host === this.options.localhost && self.options.computeAtEdge) {
        ...
        if (self.options.computeAtEdge.pathToBackend) {
          ...
          for (const [ prefix, backendCand ] of Object.entries(self.options.computeAtEdge.pathToBackend)) {
      ...
      fetchUrl = new URL(fetchUrl, `${options.useHttps ? "https" : "http"}://${host}`).toString();
    }
    ...
    try {
      ...
      if (resp.status < 400) return resp.body;
